...
export function selectActivePerformanceBudget({ budget, profileId, context }) {
  ...
  const candidates = profileId
    ...
    : document.profiles.filter((profile) => performanceBudgetScopeMatches(profile.scope, context))
  if (candidates.length === 0) {
  ...
  const scopeFailures = performanceBudgetScopeMismatches(profile.scope, context)
  if (scopeFailures.length > 0) {
    throw new ActivePerformanceBudgetError([
...
export function performanceBudgetScopeMatches(scope, context) {
  return performanceBudgetScopeMismatches(scope, context).length === 0
}
...
export function evaluateActivePerformanceBudget({ profile, metrics }) {
